...
public class VisualArrow extends VisualElement {
	 ...
	 */
	public VisualArrow(Point2D.Double newStartPoint,
			Point2D.Double newEndPoint, boolean isDirected) {
	...
	public void paintLine(Graphics2D g, Point2D.Double start,
			Point2D.Double end, Stroke stroke) {
		Shape shape = null;
		if ((start != null) && (end != null)) {
			if ((Math.abs(start.getX() - end.getX()) > 0.01)
	...
	/**
	 * @param indep
	 *            the independence of the underlying link.
	 */
	public void setIndependence(float indep) {
		INDEPENDENCE = indep;
	}
